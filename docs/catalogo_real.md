...
## Stock en vivo (2026-09-07, pedido del usuario: "stock global")

Hasta ahora `tallas_disponibles`/`tallas_variantes` en `catalog.json` eran una **foto del día** en que se cargó cada tienda (podían tener días u horas de atraso). Pedido del usuario: revalidar en vivo antes de comprar, y que esa revalidación se comparta con todos los usuarios (no solo con quien la disparó), sin golpear a las tiendas chicas en cada búsqueda.

**Verificado a mano (curl real, 2026-09-07) qué plataforma puede dar stock en vivo hoy:**
- **Shopify** (la gran mayoría de las tiendas): `<link>.js` (endpoint AJAX del propio tema, público, sin API key) siempre trae `available` por variante. Ojo: `<link>.json` NO lo trae en algunos temas (confirmado con un producto real de Doslobos) -- por eso se usa `.js`, no `.json`, para el chequeo puntual. Para refrescar TODA una tienda de una vez, `<dominio>/products.json` (paginado de a 250) sí trae `available` por variante de TODOS los productos en una sola llamada.
- **IPREX**: su API REST de WooCommerce está bloqueada, pero cada ficha real trae un bloque `<script type="application/ld+json">` tipo `Product` con `offers.availability` (confirmado con una ficha real, "Chaaqueta Bud Negra"). Solo da disponibilidad del producto **completo**, no por talla -- la propia fuente no separa más que eso.
- **RAPT (Jumpseller)**: verificado a mano -- la tienda está HOY con clave/desactivada al público ("Ingrese usando contraseña"), no hay ficha real que leer. `verificar_stock_producto()` simplemente no encuentra fuente para ese dominio y no inventa un resultado.
- **ZAMU** (2026-09-07, agregado): WooCommerce Store API pública, pero a diferencia de Shopify no da stock por variante/talla en una sola llamada -- las variantes son sub-recursos aparte y esta versión de la API no tiene un endpoint de "todas las variantes con stock" (`/products/<id>/variations` no existe acá). Con solo 9 productos no compensa pedir 1 request por variante, así que se usa el stock agregado del producto completo (`is_in_stock`/`is_purchasable`), misma granularidad que IPREX -- solo chequeo puntual al agregar al carrito, no entra en el refresco de fondo (ese es solo para Shopify).
- **Selvanegrawear y cualquier otra plataforma no reconocida**: todavía sin chequeo en vivo -- Selvanegrawear además ni siquiera tiene fotos/stock reales cargados en el catálogo todavía (ver nota más arriba), así que no tenía sentido priorizarla. Sin cambios: sigue mostrando el stock de la última recarga manual del catálogo, igual que antes.

**Arquitectura (`servicio_stock_live.py`, nuevo módulo):**
- **Nunca toca `catalog.json`** (regla del proyecto). Todo lo aprendido en vivo se guarda aparte en `data/stock_live.json` (gitignored) y se aplica como una capa encima del catálogo real (`aplicar_overlay()`), llamada desde `app.py` (`cargar_catalog_con_stock_live()`) en los 2 puntos que sirven catálogo a usuarios reales: `/api/recommend` y Descubre. Un producto sin entrada en el overlay queda exactamente como en `catalog.json` -- no hay regresión si algo falla.
- **Refresco de fondo (Shopify, cada 20 min)**: un hilo en segundo plano (arrancado una vez al iniciar `app.py`, con guard contra el reloader de Flask en modo debug) recorre las tiendas del catálogo y prueba `/products.json` en el dominio de cada una -- si responde con un JSON válido con `"products"`, se trata como Shopify y se actualizan TODOS sus productos con esa única llamada (nunca N requests por producto). Las que no respondan así (WooCommerce/Jumpseller/caídas) se saltan solas -- no hace falta mantener una lista aparte de "cuáles tiendas son Shopify".
- **Chequeo puntual al agregar al carrito** (`POST /api/verificar_stock`, `static/comun.js`): antes de guardar en el carrito local, se consulta la ficha real de esa tienda (Shopify vía `.js`, IPREX vía JSON-LD) con un botón que dice "Verificando disponibilidad..." mientras dura la consulta real (timeout de 12s, `AbortController`) -- nunca un timer inventado. Si la plataforma no tiene forma de verificar hoy (ej. RAPT), la respuesta llega con `verificado:false` y se sigue igual que antes (fail-open: nunca bloquea una compra solo porque no se pudo reconfirmar). Si la talla elegida ya no está, se avisa, se refrescan los chips de talla de la ficha y se limpia la selección -- nunca se deja "Agregado al carrito" con una talla que la tienda ya no tiene.
- **Protección contra datos viejos**: cada entrada del overlay guarda `actualizado` (timestamp) y `fuente`. A esta escala de tráfico (un solo hilo de fondo + chequeos puntuales esporádicos, todo bajo el mismo lock) nunca hay 2 refrescos del mismo producto corriendo en paralelo, así que alcanza con sobreescribir siempre con el dato más reciente -- no hizo falta lógica de reordenamiento más compleja.
- Reusa `_variantes_talla_shopify()` de `construir_catalogo_real.py` (misma función que ya usaba la carga en bloque) para no duplicar el criterio de qué cuenta como "disponible" por talla.
